...
export async function GET(request: Request) {
  ...
  try {
    ...
    const backendResponse = await fetch(`${BACKEND_API_URL}/blocks?page=${page}&limit=${limit}`, {
      method: 'GET',
      headers: {
    ...
    const fallbackResponse = await fetch('https://avail.api.subscan.io/api/scan/blocks', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    const blocks = fallbackData.data?.blocks?.map((block: any) => ({
