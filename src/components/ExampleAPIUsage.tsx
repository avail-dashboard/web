...
import { useBlocks, useChainData, useBackendStatus, useWebSocket } from '@/lib/hooks/useAvailAPI'
import { BackendStatus, StatusBadge } from './BackendStatus'

export function ExampleAPIUsage() {
  // Using the new API hooks
  const { data: blocks, loading: blocksLoading, error: blocksError, refetch } = useBlocks(5, {
    ...
    <div className="p-6 space-y-6">
        ...
        {chainLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <div className="text-xl font-semibold">{chainData.finalizedBlocks.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-sm text-gray-500">Total Accounts</div>
              <div className="text-xl font-semibold">{chainData.totalAccounts.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-sm text-gray-500">Token Price</div>
              <div className="text-xl font-semibold">${chainData.tokenPrice.toFixed(4)}</div>
            </div>
            <div>
              <div className="text-sm text-gray-500">24h Change</div>
              <div className={`text-xl font-semibold ${chainData.priceChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      <div className="bg-white rounded-lg border p-4">
        ...
        {blocksLoading ? (
          ...
          <div className="space-y-2">
            {blocks.map((block) => (
              <div key={block.hash} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                <div>
      ...
      <div className="bg-gray-50 rounded-lg p-4">
        ...
        <div className="space-y-2 text-sm">
          <div><strong>Frontend-Backend Integration:</strong> ✅ Active</div>
          <div><strong>Automatic Fallback:</strong> ✅ Configured</div>
          <div><strong>Real-time Updates:</strong> {wsConnected ? '✅ Connected' : '❌ Disconnected'}</div>
          <div><strong>Error Handling:</strong> ✅ Implemented</div>
          <div><strong>Loading States:</strong> ✅ Managed</div>
          <div><strong>Auto-refresh:</strong> ✅ Configured</div>
        </div>
      </div>
    </div>
  )
